...
class MouvementStock(models.Model):
    ...
    def clean(self):
        ...
        # Un mouvement doit concerner soit un matériel, soit un consommable.
        if not self.id_materiel and not self.id_consommable:
            errors["id_materiel"] = "Le mouvement doit concerner soit un matériel, soit un consommable."

        if self.id_materiel and self.id_consommable:
            errors["id_materiel"] = "Le mouvement ne peut pas concerner un matériel et un consommable en même temps."

        # Règles selon le type de mouvement.
        if self.type_mouvement == self.TypeMouvement.ENTREE:
            if not self.magasin_destination:
                errors["magasin_destination"] = "Une entrée doit avoir un magasin de destination."
        ...
        if self.type_mouvement == self.TypeMouvement.TRANSFERT:
            ...
            if self.magasin_source and self.magasin_destination:
                if self.magasin_source == self.magasin_destination:
                    errors["magasin_destination"] = "Le magasin source et le magasin destination doivent être différents."
    ...
    def clean(self):
        if self.date_retour and self.date_retour < self.date_affectation:
            raise ValidationError(
                "La date de retour ne peut pas être inférieure à la date d'affectation."
            )
