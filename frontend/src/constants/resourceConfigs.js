...
function materialCanMove({ option, values }) {
  if (values.type_mouvement !== "SORTIE") return true;
  return option.item?.statut_stock === "EN_STOCK" && option.item?.etat !== "HORS_SERVICE";
}

function consommableCanLeaveStock({ option, values }) {
  if (values.type_mouvement !== "SORTIE") return true;
  return Number(option.item?.quantite_stock || 0) > 0;
}
...
export const resourceConfigs = {
  ...
  materiels: {
    ...
    formGroups: [
      ...
      { title: "Stock et etat", tone: "green", fields: ["statut_stock", "etat", "code_barre", "qr_code"] },
    ...
    formSteps: [
      ...
      { title: "Stock", tone: "slate", fields: ["statut_stock", "etat", "duree_garantie_mois", "garantie_fin"] },
      { title: "Notes", tone: "slate", fields: ["observation"] },
    ],
    detailGroups: [
      { title: "Identification", tone: "blue", fields: ["code_materiel", "numero_serie", "marque", "modele", "categorie_nom", "famille_nom"] },
      { title: "Stock et fournisseur", tone: "green", fields: ["statut_stock", "etat", "fournisseur_nom"] },
  ...
  consommables: {
    ...
    formGroups: [
      ...
      { title: "Stock", tone: "green", fields: ["quantite_stock", "seuil_alerte", "statut"] },
    ],
    formSteps: [
      { title: "Famille", tone: "blue", fields: ["id_famille", "id_categorie"] },
      { title: "Consommable", tone: "green", fields: ["nom_consommable", "id_unite"] },
      { title: "Stock", tone: "orange", fields: ["quantite_stock", "seuil_alerte", "statut"] },
    ],
    detailGroups: [
      { title: "Identification", tone: "blue", fields: ["code_consommable", "nom_consommable", "categorie_nom", "famille_nom", "unite_nom"] },
      { title: "Stock", tone: "green", fields: ["quantite_stock", "seuil_alerte", "statut", "date_creation"] },
    ],
  },
  mouvements: {
    title: "Mouvements",
    description: "Entrees, sorties et ajustements de stock.",
    ...
    fields: [
      { name: "type_mouvement", label: "Type", type: "select", required: true, options: ["ENTREE", "SORTIE", "AJUSTEMENT"] },
      { name: "id_materiel", label: "Materiel", type: "recordPicker", resource: "materiels", requiredWhen: fieldMissing("id_consommable"), availableWhen: materialCanMove, disabledWhen: anyRule(needsMovementType, fieldHasValue("id_consommable")), searchPlaceholder: "Rechercher un materiel..." },
      { name: "id_consommable", label: "Consommable", type: "recordPicker", resource: "consommables", requiredWhen: fieldMissing("id_materiel"), availableWhen: consommableCanLeaveStock, disabledWhen: anyRule(needsMovementType, fieldHasValue("id_materiel")), searchPlaceholder: "Rechercher un consommable..." },
  ...
  affectations: {
    ...
    fields: [
      ...
      { name: "id_materiel", label: "Materiel disponible", type: "recordPicker", required: true, resource: "materiels", filterOptions: materialBelongsToSelectedCategory, availableWhen: materialIsAvailableForAffectation, disabledWhen: needsSelectedCategory, searchPlaceholder: "Rechercher par code, marque, modele, categorie...", emptyText: "Aucun materiel disponible pour cette categorie." },
      { name: "entite_type", label: "Type d'entite", type: "select", required: true, options: ["DEPARTEMENT", "DIRECTION", "AGENT"], clears: ["entite_id", "agent_id_departement", "agent_id_direction", "agent_matricule", "agent_nom_complet", "agent_telephone"] },
      { name: "entite_id", label: "Destinataire", type: "recordPicker", required: true, resource: resourceForEntiteType, optionResources: ["departements", "directions"], visibleWhen: isNotAgentAffectation, disabledWhen: needsEntiteType, searchPlaceholder: "Rechercher le destinataire..." },
  ...
  inventaires: {
    ...
    fields: [
      ...
      { name: "entite_type", label: "Type d'entite", type: "select", required: true, options: ["DEPARTEMENT", "DIRECTION"] },
      { name: "entite_id", label: "Perimetre", type: "recordPicker", required: true, resource: resourceForEntiteType, optionResources: ["departements", "directions"], disabledWhen: needsEntiteType, searchPlaceholder: "Rechercher le perimetre..." },
  ...
  demandes: {
    ...
    fields: [
      ...
      { name: "origine_type", label: "Origine", type: "select", options: ["DIRECTION", "DEPARTEMENT"], defaultValue: "DIRECTION", disabledWhen: currentUserIsNotAdmin },
  ...
  users: {
    ...
    fields: [
      ...
      { name: "scope_type", label: "Perimetre", type: "select", options: ["GENERAL", "DEPARTEMENT", "DIRECTION"], defaultValue: "GENERAL", disabledWhen: selectedRoleIsFixedGeneral, disabledValue: "GENERAL" },
    ...
    formSteps: [
      ...
      { title: "Perimetre", tone: "green", fields: ["id_departement", "id_direction"] },
